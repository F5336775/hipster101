...
class PointsResourceIT {
    ...
    void createPoints() throws Exception {
        ...
        var returnedPointsDTO = om.readValue(
            restPointsMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(pointsDTO)))
        ...
        var returnedPoints = pointsMapper.toEntity(returnedPointsDTO);
    ...
    void getAllPointsWithEagerRelationshipsIsEnabled() throws Exception {
        when(pointsServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restPointsMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(pointsServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void putExistingPoints() throws Exception {
        ...
        restPointsMockMvc
            .perform(
                put(ENTITY_API_URL_ID, pointsDTO.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(pointsDTO))
            )
            .andExpect(status().isOk());
        ...
        restPointsMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    ...
                    .content(om.writeValueAsBytes(pointsDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamPoints() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPointsMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(pointsDTO)))
    ...
    void patchWithMissingIdPathParamPoints() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPointsMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(pointsDTO)))
            .andExpect(status().isMethodNotAllowed());

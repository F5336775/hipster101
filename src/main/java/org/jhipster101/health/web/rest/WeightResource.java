...
import org.jhipster101.health.service.WeightQueryService;
...
/**
 * REST controller for managing {@link org.jhipster101.health.domain.Weight}.
 */
@RestController
...
public class WeightResource {
    ...
    public ResponseEntity<WeightDTO> createWeight(@Valid @RequestBody WeightDTO weightDTO) throws URISyntaxException {
        ...
        weightDTO = weightService.save(weightDTO);
        return ResponseEntity.created(new URI("/api/weights/" + weightDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, weightDTO.getId().toString()))
            .body(weightDTO);
    }
    ...
    public ResponseEntity<WeightDTO> updateWeight(
        ...
        weightDTO = weightService.update(weightDTO);
    ...
    public ResponseEntity<List<WeightDTO>> getAllWeights(
        ...
        Page<WeightDTO> page = weightQueryService.findByCriteria(criteria, pageable);
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<WeightDTO> getWeight(@PathVariable("id") Long id) {
     ...
     */
    @GetMapping("/_search")
    public ResponseEntity<List<WeightDTO>> searchWeights(
